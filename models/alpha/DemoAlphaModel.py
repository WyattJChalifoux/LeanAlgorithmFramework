...
class DemoAlphaModel(AlphaModel):
            ...
            if symbolData.Fast.IsReady and symbolData.Slow.IsReady:
                ...
                if symbolData.FastIsOverSlow:
                    if symbolData.Slow > symbolData.Fast:
                        algorithm.Debug("ALPHA MODEL:: Generating insight: " + str(Insight.Price(symbolData.Symbol, self.predictionInterval, InsightDirection.Down)))
                        insights.append(
                            Insight.Price(symbolData.Symbol, self.predictionInterval, InsightDirection.Down, magnitude=-10))

                elif symbolData.SlowIsOverFast:
                    if symbolData.Fast > symbolData.Slow:
                        algorithm.Debug("ALPHA MODEL:: Generating insight: " + str(Insight.Price(symbolData.Symbol, self.predictionInterval, InsightDirection.Up)))
                        insights.append(Insight.Price(symbolData.Symbol, self.predictionInterval, InsightDirection.Up, magnitude=10))
...
class SymbolData:
    ...
    def __init__(self, security, fastPeriod, slowPeriod, algorithm, resolution):
        ...
        self.Fast = ExponentialMovingAverage(security.Symbol, fastPeriod,
